// notify-new-order production function with optional admin-only debug actions (fixed)
...
serve(async (req: Request) => {
  try {
    ...
    if (!SUPABASE_URL || !SUPABASE_KEY) {
      ...
      return new Response(JSON.stringify({ error: 'Missing SUPABASE_URL or SUPABASE_KEY' }), {
        status: 500,
    ...
    if (ADMIN_TOKEN && adminHeader && adminHeader === ADMIN_TOKEN) {
      ...
      if (action === 'insert_test') {
        ...
        return new Response(JSON.stringify({ ok: ins.ok, status: ins.status, body }), {
          status: ins.ok ? 201 : 500,
          headers: { 'content-type': 'application/json' },
        });
      }
    ...
    if (!res.ok) {
      ...
      return new Response(
        JSON.stringify({ error: 'Failed to fetch subscriptions', detail: text }),
    ...
    for (const row of subs) {
      ...
      try {
        ...
        if (
          ...
          subscription.endpoint.includes('fcm')
        ) {
          await fetch('https://fcm.googleapis.com/fcm/send', {
            ...
            body: JSON.stringify({
              ...
              notification: { title: 'New order', body: `Order: ${payload?.record?.total ?? ''}` },
            }),
          });
        } else {
    ...
    return new Response(JSON.stringify({ ok: true, delivered: subs.length }), {
      status: 200,
      headers: { 'content-type': 'application/json' },
    });
  } catch (err) {
