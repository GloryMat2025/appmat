...
(function(){
  ...
  function readDraft(){ try { return JSON.parse(localStorage.getItem(LS_DRAFT)||'{}'); } catch { return {}; } }
  ...
  function buildOrder(){
    ...
    let outletId = null;
    try { const sel = JSON.parse(localStorage.getItem('gf_pickup_outlet')||'null'); if(sel && sel.id) outletId = sel.id; } catch {}
    return {
  ...
  async function submitOrder(){
    ...
    try {
      // Integrate with global offline queue if exists
      if(window.__offlineQueue && !navigator.onLine){
        window.__offlineQueue.enqueue({ type:'placeOrder', order });
        announce('Order queued for sync when online');
        afterSuccess(order, true);
      } else if(window.__offlineQueue){
        window.__offlineQueue.enqueue({ type:'placeOrder', order });
        // attempt immediate replay
        try { await window.__offlineQueue.replay(); } catch {}
    ...
    } catch(e){ console.error(e); alert('Failed submitting order'); els.pay.disabled=false; els.pay.textContent='Pay Now'; }
  }
