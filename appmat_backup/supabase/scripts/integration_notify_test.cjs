...
(async () => {
  try {
    ...
    const testSub = { subscription: { endpoint: 'https://example.com/push/integration-test', keys: { p256dh: 'p256', auth: 'auth' } } };
    ...
    let inserted;
    try { inserted = JSON.parse(insertText)[0]; } catch (e) { inserted = null; }
    ...
    // 2) Trigger function
    const funcUrl = (env.SUPABASE_URL || SUPABASE_URL).replace('.supabase.co', '.functions.supabase.co') + '/notify-new-order-fixed';
    ...
    const delRes = await fetch(`${SUPABASE_URL.replace(/\/$/, '')}/rest/v1/push_subscriptions?id=eq.${inserted.id}`, {
      method: 'DELETE',
