...
const dirs = fs
  ...
  .filter((n) => fs.statSync(path.join(shotsDir, n)).isDirectory());
...
<div style="display:flex;flex-wrap:wrap">${rows
  .map(
    (r) =>
      `<figure style="margin:8px"><img src="${r.img}" style="max-width:320px"><figcaption>${r.dir}</figcaption></figure>`
  )
  .join('\n')}</div>
</body></html>`;
